...
public class MusicFavorisController {
    ...
    */
    ...
    // ── NOUVEAU ENDPOINT : RÉCUPÉRER LES GENRES FAVORIS D'UN UTILISATEUR (cote 10 et 7) ──

    /**
     * GET /api/music-favoris/users/{userId}/favorite-genre-10-7
     * Récupère la liste des genres favoris d'un utilisateur pour les cotes 10/10 et 7/10
     *
     * @param userId L'ID de l'utilisateur
     * @return ResponseEntity contenant les genres pour cote 10 et cote 7
     */
    @GetMapping("/users/{userId}/favorite-genre-10-7")
    public ResponseEntity<Map<String, Object>> getUserFavoriteGenres10and7(@PathVariable Long userId) {
        try {
            // 1. Vérifier que l'utilisateur existe
            User user = userService.findById(userId)
                    .orElseThrow(() -> new RuntimeException("Utilisateur non trouvé avec l'ID: " + userId));

            // 2. Récupérer tous les MusicFavoris de l'utilisateur
            List<MusicFavoris> favorisList = musicFavorisService.getMusicFavorisByUserId(userId);

            // 3. Initialiser les structures de réponse
            Map<String, Object> response = new HashMap<>();
            List<String> genresCote10 = new ArrayList<>();
            List<String> genresCote7 = new ArrayList<>();

            // 4. Parcourir les favoris pour trouver ceux avec cote 10 et cote 7
            for (MusicFavoris favoris : favorisList) {
                ...
                // Récupérer les descriptions des genres
                List<String> genreDescriptions = genres.stream()
                        .map(MusicGenre::getDescription)
                        .filter(desc -> desc != null && !desc.trim().isEmpty())
                        .collect(Collectors.toList());

                if (cote == 10) {
                    genresCote10.addAll(genreDescriptions);
                } else if (cote == 7) {
                    genresCote7.addAll(genreDescriptions);
                }
            }
            ...
            response.put("genres_cote_10", genresCote10);
            response.put("genres_cote_7", genresCote7);
            response.put("count_cote_10", genresCote10.size());
            response.put("count_cote_7", genresCote7.size());
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Map.of(
                            "success", false,
                            "error", "Erreur lors de la récupération des genres favoris: " + e.getMessage()
                    ));
        }
    }

    // ── NOUVEAU ENDPOINT : RÉCUPÉRER LES ARTISTES PAR GENRE ───────────────────────

    /**
     * GET /api/music-favoris/artists/by-genre
     * Récupère la liste des artistes (Users avec role ARTISTE) qui ont un MusicFavoris
     * avec cotePreference = 9 et dont les genres correspondent au genre spécifié.
     *
     * @param genre Le nom du genre recherché (ex: "Pop", "Rock", "Hip-Hop")
     * @return ResponseEntity contenant la liste des artistes
     */
    @GetMapping("/artists/by-genre")
    public ResponseEntity<Map<String, Object>> getArtistsByGenre(@RequestParam String genre) {
        try {
            if (genre == null || genre.trim().isEmpty()) {
                return ResponseEntity.badRequest()
                        .body(Map.of("success", false, "error", "Le paramètre 'genre' est requis"));
            }

            // 1. Récupérer tous les artistes (users avec role ARTISTE)
            List<User> allArtists = userService.getUsersByRole("ARTISTE");

            // 2. Filtrer les artistes qui ont un MusicFavoris avec cote 9 contenant le genre recherché
            List<Map<String, Object>> matchingArtists = new ArrayList<>();

            for (User artist : allArtists) {
                // Récupérer les MusicFavoris de l'artiste
                List<MusicFavoris> favorisList = musicFavorisService.getMusicFavorisByUserId(artist.getId());

                // Chercher un MusicFavoris avec cotePreference = 9
                for (MusicFavoris favoris : favorisList) {
                    if (favoris.getCotePreference() == 9) {
                        List<MusicGenre> genres = favoris.getGenres();
                        if (genres != null && !genres.isEmpty()) {
                            // Vérifier si le genre recherché est dans la liste
                            boolean hasGenre = genres.stream()
                                    .anyMatch(g -> g.getDescription() != null
                                            && g.getDescription().equalsIgnoreCase(genre.trim()));
                            ...
                            if (hasGenre) {
                                ...
                                break; // On sort de la boucle des favoris car on a trouvé une correspondance
                            }
                        }
                    }
                }
            ...
            response.put("count", matchingArtists.size());
            response.put("artists", matchingArtists);
